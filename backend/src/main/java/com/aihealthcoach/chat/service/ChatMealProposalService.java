...
public class ChatMealProposalService {
    ...
    public ConfirmMealProposalResponse confirm(Long userId, ConfirmMealProposalRequest request) {
        ...
        var dailyMeal = mealService.createMeal(userId, createMealRequest);
    ...
    private String toConfirmationMessage(String mealType) {
        return switch (mealType) {
            case "BREAKFAST" -> "아침 식단으로 기록했어요.";
            case "LUNCH" -> "점심 식단으로 기록했어요.";
            case "DINNER" -> "저녁 식단으로 기록했어요.";
            case "SNACK" -> "간식 식단으로 기록했어요.";
            default -> "식단으로 기록했어요.";
        };
    }
}
