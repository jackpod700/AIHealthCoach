...
| 회원 | 건강 프로필 조회 | 키, 현재 체중, 목표 체중, 목표 유형, 성별, 나이, 수정 시각을 조회한다. | F08 |
| 회원 | 건강 프로필 수정 | 건강 프로필 항목을 수정하여 목표 계산과 AI 코칭에 반영한다. | F09 |
| 회원 | 채팅 이력 조회 | 사용자 메시지와 AI 응답 이력을 조회한다. | F10 |
| 회원 | 텍스트 AI 채팅 | 텍스트 메시지를 보내고 AI 응답을 받는다. | F11 |
| 회원 | 스트리밍 AI 응답 | AI 응답 생성 중 부분 응답을 실시간으로 확인한다. | F12, N15 |
| 회원 | 이미지 AI 채팅 | 이미지와 선택 텍스트를 전송하여 AI 응답과 기록 제안을 받는다. | F13, N18 |
| AI 서비스 | AI 컨텍스트 활용 | 프로필, 목표, 기록, 최근 대화, 요약, 메모리를 AI 응답 컨텍스트로 활용한다. | F14, N14 |
| AI 서비스 | 식단 정보 추출 | 채팅 내용에서 식사 날짜, 식사 유형, 음식명, 수량을 추출한다. | F15 |
| 회원 | AI 식단 제안 확정 | AI가 제안한 음식 후보와 수량을 확인하고 식단으로 저장한다. | F16 |
| AI 서비스 | 운동 정보 추출 | 채팅 내용에서 운동명, 강도, 날짜, 시간, 메모를 추출한다. | F17 |
| 회원 | AI 운동 제안 확정 | AI가 제안한 운동 후보와 강도, 시간을 확인하고 운동 기록으로 저장한다. | F18 |
| AI 서비스 | 체중 정보 추출 | 채팅 내용에서 측정 날짜와 체중 값을 추출한다. | F19 |
| 회원 | AI 체중 제안 확정 | AI가 제안한 체중 값을 확인하고 체중 기록으로 저장한다. | F20 |
| AI 서비스 | 채팅 기반 개인화 기억 저장 | 사용자의 기억 저장 요청을 식별하고 저장 가능 여부를 판단한다. | F21 |
| 회원 | 일일 목표 추천 | 목표 유형별 권장 섭취 칼로리와 운동 칼로리를 조회한다. | F22 |
| 회원 | 일일 목표 확정 및 수정 | 목표 유형, 섭취 칼로리 목표, 운동 칼로리 목표를 저장한다. | F23 |
| 회원 | 일일 목표 진행률 조회 | 날짜별 섭취/운동 목표 달성률과 영양 비율을 조회한다. | F24 |
| 회원 | 음식 목록 검색 | 음식명 키워드로 음식 DB와 제공량별 영양 정보를 검색한다. | F25 |
| 회원 | 식단 입력용 음식 후보 검색 | 식단 작성 또는 AI 제안 확정에 사용할 음식 후보를 검색한다. | F26 |
| 회원 | 음식 검색 실패 기록 | 검색 결과가 없는 키워드를 후보 수집 대상으로 남긴다. | F27 |
| 외부 음식 API | 외부 음식 후보 수집 | 검색 실패 데이터를 기반으로 외부 음식 후보를 제공한다. | F28 |
| 회원 | 음식 등록 요청 작성 | DB에 없는 음식의 이름과 영양 정보를 등록 요청한다. | F29 |
| 회원 | 식단 작성 및 수정 | 날짜, 식사 유형, 음식, 수량으로 식단을 저장하거나 갱신한다. | F30 |
| 회원 | 식단 삭제 | 본인이 작성한 식단 기록을 삭제한다. | F31 |
| 회원 | 일일 식단 조회 | 특정 날짜의 식단과 영양 합계를 조회한다. | F32 |
| 회원 | 월간 식단 조회 | 월별 날짜 단위 식단 요약을 조회한다. | F33 |
| 회원 | 운동 종목 검색 | 운동명 키워드로 종목과 강도별 MET 정보를 검색한다. | F34 |
| 회원 | 운동 기록 작성 | 운동 종목, 강도, 날짜, 시간, 메모로 운동을 기록한다. | F35 |
| 회원 | 운동 기록 수정 | 본인이 작성한 운동 기록의 종목, 강도, 시간, 메모를 수정한다. | F36 |
| 회원 | 운동 기록 삭제 | 본인이 작성한 운동 기록을 삭제한다. | F37 |
| 회원 | 일일 운동 기록 조회 | 특정 날짜의 운동 기록과 소모 칼로리를 조회한다. | F38 |
| 회원 | 월간 운동 기록일 조회 | 특정 월에 운동 기록이 있는 날짜 목록을 조회한다. | F39 |
| 회원 | 체중 기록 저장 및 수정 | 날짜별 체중을 저장하거나 수정한다. | F40 |
| 회원 | 체중 기록 조회 | 전체 또는 기간별 체중 기록과 추세를 조회한다. | F41 |
| 회원 | 체중 기록 삭제 | 특정 날짜의 체중 기록을 삭제한다. | F42 |
| Babtory 시스템 | 현재 체중 동기화 | 체중 기록 변경 후 건강 프로필의 현재 체중을 최신 기록과 맞춘다. | F43 |
| 회원 | 월간 캘린더 조회 | 월간 캘린더에서 식단, 운동, 체중 기록 요약을 확인한다. | F44 |
| 회원 | 일별 기록 통합 관리 | 선택한 날짜의 식단, 운동, 체중을 함께 조회하고 관리한다. | F45 |
| 관리자 | 관리자 대시보드 조회 | 전체 사용자 수, 당일 가입자 수, 최근 활성 사용자 수를 확인한다. | F46 |
| 관리자 | 음식 등록 요청 목록 및 상세 조회 | 사용자 음식 등록 요청을 상태/페이지 조건으로 조회하고 상세를 확인한다. | F47 |
| 관리자 | 음식 등록 요청 승인 | 사용자 제출 음식을 검토/보정하여 공식 음식 데이터로 승인한다. | F48 |
| 관리자 | 음식 등록 요청 반려 | 부적절하거나 중복된 음식 등록 요청을 반려 사유와 함께 처리한다. | F49 |
| 관리자 | 외부 음식 후보 조회 | 검색 실패 기반으로 수집된 외부 음식 후보 그룹을 조회한다. | F50 |
| 관리자 | 외부 음식 후보 승인 | 외부 음식 후보를 공식 음식 데이터에 반영한다. | F51 |
| 관리자 | 외부 음식 후보 반려 | 외부 음식 후보 그룹을 반려 사유와 함께 처리한다. | F52 |
| 운영자 | 헬스 체크 | 서비스 생존 여부와 응답 시각을 확인한다. | F53, N19 |
| 운영자 | 외부 음식 API 연동 테스트 | FatSecret 음식 검색 연동 상태와 응답을 확인한다. | F54, N24 |
...
- 현재 `requirements.md`의 기능 요구사항 번호는 `F01`부터 `F54`까지 연속 번호로 정리되어 있으며, 본 문서의 매핑 표도 해당 번호 체계를 따른다.
